...
export async function getStaticProps({ params }) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/Servicio/${sid}`);
    const data = await res.json();
...
function ServicioDetail  ({ servicio = {} }) {
    ...
    async function handleDelete() {
        ...
        const confirmed = window.confirm('¿Estás seguro de que quieres eliminar este servicio?');
        if (confirmed) {
            setIsLoading(true);
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/Servicio/${servicio.Id}`, {
                method: 'DELETE'
            });
            if (res.ok) {
                ...
                setIsLoading(false);
                console.error('Error deleting service:', res.status);
            }
        }
    }
        ...
        <>
            <Dashboard>
                <div className="h-screen flex flex-col justify-start items-center bg-gray-100">
                    ...
                    <div className="mt-6">
                        <button disabled={isLoading}  className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded">
                            Editar
                        </button>



                            <button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 ml-4 rounded" onClick={handleDelete} disabled={isLoading}>
                                {isLoading ?
                                    <>
