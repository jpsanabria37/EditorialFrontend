...
export async function getStaticProps({ params }) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/TipoDocumento/${tid}`
  );
  const data = await res.json();
...
function TipoDocumentoDetalle({ tipoDocumento = {} }) {
  ...
  async function handleDelete() {
    ...
    if (confirmed) {
      setIsLoading(true);
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/TipoDocumento/${tipoDocumento.Id}`,
        {
          method: "DELETE",
        }
      );
      if (res.ok) {
        router.push("/tipodocumentos");
      }
      setIsLoading(false);
      console.error("Error deleting TipoDocumento:", res.status);
    }
  }
...
export default TipoDocumentoDetalle;
